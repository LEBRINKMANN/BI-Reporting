...
From   Dw.Line_Item_Details lid
       join DW.LINE_ITEM_STATUS_SUMMARY SS On SS.LINE_ITEM_ID = lid.LINE_ITEM_ID 
       join DW.BILLING_TRANSACTIONS b on b.line_item_id = lid.LINE_ITEM_ID and b.TRANSACTION_MULTIPLIER = 1
       join DW.CUSTOMERS c on c.COMPANY_ID = lid.CUSTOMER_ID 
       join DW.SHIPPING_INFO s On lid.LINE_ITEM_ID = s.LINE_ITEM_ID
...
where  ss.last_ordered_timestamp is not null 
       ...
       and (lid.ESD_DESCRIPTION = 'Backordered' or b.BACKORDER_IND = 1)
       and c.Lvl1_Company_Id  in (119, 60891, 4782)
Group By c.LVL1_COMPANY_NAME
with ur;
...
Select 'Critical Item - Not Ordered', c.LVL1_COMPANY_NAME, Count(distinct lid.line_item_id) as Status_Count
From   Dw.Line_Item_Details lid
       join DW.LINE_ITEM_STATUS_SUMMARY SS On SS.LINE_ITEM_ID = lid.LINE_ITEM_ID 
       join DW.CUSTOMERS c on c.COMPANY_ID = lid.CUSTOMER_ID 
where  ss.last_ordered_timestamp is not null 
...
Select 'Critical Item - Awaiting Sourcing', c.LVL1_COMPANY_NAME, Count(distinct lid.line_item_id) as Status_Count
From   Dw.Line_Item_Details lid
       join DW.LINE_ITEM_STATUS_SUMMARY SS On SS.LINE_ITEM_ID = lid.LINE_ITEM_ID 
       join DW.CUSTOMERS c on c.COMPANY_ID = lid.CUSTOMER_ID 
where  ss.last_ordered_timestamp is not null 
...
From   Dw.Line_Item_Details lid
       join DW.LINE_ITEM_STATUS_SUMMARY SS On SS.LINE_ITEM_ID = lid.LINE_ITEM_ID 
       join DW.CUSTOMERS c on c.COMPANY_ID = lid.CUSTOMER_ID 
       join (select count(updated_timestamp) as updated_ship_count, 
                    LINE_ITEM_ID 
...
where  ss.last_ordered_timestamp is not null 
       and SS.LAST_ORDERED_TIMESTAMP >= current_timestamp - 3
       and t.DATE_SHIPPED > lid.ESTIMATED_SHIPPING_DATE 
       and c.Lvl1_Company_Id in (119, 60891, 4782)
Group By c.LVL1_COMPANY_NAME
with ur;
...
Select 'Item - Backordered w/ ESD', c.LVL1_COMPANY_NAME, Count(distinct lid.line_item_id) as Status_Count
From   Dw.Line_Item_Details lid
       join DW.LINE_ITEM_STATUS_SUMMARY SS On SS.LINE_ITEM_ID = lid.LINE_ITEM_ID 
       left join DW.BILLING_TRANSACTIONS b on b.LINE_ITEM_ID = lid.LINE_ITEM_ID and b.TRANSACTION_MULTIPLIER = 1
       join DW.CUSTOMERS c on c.COMPANY_ID = lid.CUSTOMER_ID 
where  ss.last_ordered_timestamp is not null 
       and SS.LAST_ORDERED_TIMESTAMP >=  current_timestamp - 3
       and lid.ESTIMATED_SHIPPING_DATE is not null 
